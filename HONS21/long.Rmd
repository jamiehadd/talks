...
# ***When is community detection possible?*** 
...
.alert[**Experimental results**] in random hypergraphs.
  ...
  .row.bg-main5[.content[.font_medium[
...
In a stochastic blockmodel with two, equally-sized communities:
...
Approximates to the likelihood of a degree-corrected hypergraph blockmodel with "All-Or-Nothing" affinities.  
]
...
1. All nodes start in their own clusters. 
  - Greedily agglomerate nodes to maximize $Q$. 
  - Greedily agglomerate **clusters** of nodes to further maximize $Q$. 
2. Re-estimate $\beta_k$ and $\gamma_k$. 
3. Repeat...
...
<br> <br> 
Unclear! Extant theory on hypergraphs only treats *uniform* hypergraphs (all edges same size).

Most hypergraph data sets are non-uniform...

We seek **.alert[detectability theory for non-uniform hypergraphs].**
...
## A few papers treating uniform hypergraphs
...
The .alert[hypergraph nonbacktracking operator] $\color{#ff5252}{\mathbf{B}}$ is the linear operator 
...
"Rediscovered" and used for community detection by Angelini, M. C., Caltagirone, F., Krzakala, F., & Zdeborová, L. (2015), *Allerton Conference*.



 **Well-defined** on non-uniform hypergraphs!
...
Surprisingly disappointing performance in experiments...

We know experimentally that detection is possible in the ? corners... 
...
We use a modified nonbacktracking operator to adapt to .alert[qualitatively distinct roles] for edges of multiple sizes.

The community-correlated eigenvector can either the first or second eigenvector of the modified operator. 

Under some approximations, we can analytically derive the undetectable region: 
...
Fast computation via Ihara-Bass determinants (reduce the effective size of $\mathbf{B}$).
...
Bounds on spectral clustering $\stackrel{?}{\implies}$ bounds on any algorithm?

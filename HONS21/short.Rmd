...
**Theorem [PC, JH, NE 2021] (Ihara Bass Determinant Formula)**: Under mild restrictions, the "interesting" eigenvalues of $\mathbf{B} \in \mathbb{R}^{m\langle k\rangle \times m\langle k\rangle}$ coincide with the eigenvalues of the matrix 
 
$$
\mathbf{B}' = \left[\begin{matrix}
  <!-- \mathbf{0} & \mathbf{D} - \mathbf{I}_{\bar{k}n} \\ -->
  <!--   (\mathbf{I}_{\bar{k}}- \mathbf{K}) \otimes \mathbf{I}_n &  \mathbf{A} + (2\mathbf{I}_{\bar{k}} - \mathbf{K})\otimes \mathbf{I}_n -->
\end{matrix}\right] \in \mathbb{R}^{2\bar{k}n\times 2\bar{k}n}\;.
$$

- $\mathbf{A}$ holds "basic" adjacency information.
- $\mathbf{D}$ holds node degrees. 
- $\mathbf{K}$ holds edge sizes. 

...
We could construct a set of adjacency tensors $\mathbf{A}^{(2)}$, $\mathbf{A}^{(3)}$...
...
Can represent edges of all sizes in the same matrix!
...
So, here is the story I have told you so far:
]
...
(If $\lambda_2$ is .alert[complex] then this doesn't work, and community detection turns out to be impossible!)
]
...
4. Use a Euclidean clustering algorithm (like $k$-means) in the space spanned by the vectors $\{\mathbf{u}\}$. 
...
\mathbf{0} & \mathbf{I}_\ell \otimes \bar{\mathbb{D}} \\ 
\mathbf{0} & \mathbf{I}_\ell \otimes \bar{\mathbb{A}}
...
<br> <br> <br> <br> <br> 
## Ok, but what about .alert[real data]?
...
<br> <br> 
  ...
  - $m = 7,818$ social contact events (edges) measured by wearable sensors.   
...
.column.bg-main1[
  ...
  Recall that we had this *suspiciously round* region where our algorithm totally failed to learn any cluster information. 
...
.column.bg-main1[
  ...
  Strategy: make some approximations and ask when $\mathbf{J}$ has an eigenvalue $> 1$. 

  Future work: justify the approximations. 
...
We conjecture the same thing for hypergraphs: inside that ellipse, the clustering problem is not just practically but *theoretically* impossible. 
...
The Hashimoto operator lets us use eigenvector techniques for clustering hypergraphs. 

Determinant identities let us speed up computation. 

Some hypergraph data sets "need" hypergraph methods, while others don't. 

There are lots of open questions around the fundamental limits of hypergraph clustering. 

---
title: "Eigenvector Methods for Clustering Hypergraphs"                 
subtitle: "Applied Math Seminar, CCMS"
author: "<span class =     'author-highlight'>Phil Chodrow</span> <br> UCLA Mathematics <br> Sept 20th, 2021"          
date: ""              
output:                       
...
.alert2[**Graph clustering**] with eigenvectors.
  ]]]     
...
.alert[**Open questions**] and future work. 
  ]]]
]] 
...
<br><br><br>
  <img src="img/opportunity.jpeg" width=100%>
...
- $\color{#FFD046}{c_i}$: average number of .alert2[within-cluster] neighbors per node.
- $\color{#EE6C4D}{c_o}$: average number of .alert[between-cluster] neighbors per node. 

**Detectability threshold**: In the large-graph limit, it is possible to *detect communities* iff  

$$(\color{#FFD046}{c_i} - \color{#EE6C4D}{c_o})^2 > \frac{1}{2}\sqrt{\color{#FFD046}{c_i} + \color{#EE6C4D}{c_o}}\;.$$  
...
# <br> <br> 
Informally, an algorithm .alert[**detects communities**] if the output of the algorithm has better-than-random correlation with true communities (in expectation). 
...
.column.bg-main1[
## Eigenvectors to Clusters <br> 

<br>
1. Arrange the top $k$ eigenvectors in a matrix $\mathbf{M}$, $k$ should be close to the desired number of clusters. 
2. Threshold: let $\bar{m}_{ij} = \mathbb{1}[m_{ij} > 0]$. 
2. Perform k-means or your personal favorite Euclidean clustering algorithm on $\bar{\mathbf{M}}$. 

<br> 

Visualization of true clusters (classes) in a high-school contact hypergraph $\rightarrow$ 

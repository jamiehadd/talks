...
subtitle: "Workshop on Computational Harmonic Analysis and Linear Algebra<br> Focus Program on Data Science, Approximation Theory, and Harmonic Analysis<br> The Fields Institute for Research in Mathematical Sciences <br>May 12th, 2022"
...
# xaringan::inf_mr("CISS_2022/index.rmd")
```
...
Average consensus problem and the .alert[block gossip method].
  ]]]     
  .row.bg-main2[.content.vmiddle[.font_medium[
Matrix row pavings and the .alert[**block Kaczmarz method**].  
  ]]] 
  .row.bg-main3[.content.vmiddle[.font_medium[ 
.alert[**Convergence results**] and a bridge between dynamics on networks and NLA. 
  ]]]
]] 
...
.column[.lil-stretch[<br><br>
  <img src="img/ben.jpg" width=80%> 
  ]]
.column[.lil-stretch[<br><br>
  <img src="img/temp.jpeg" width=80%> 
]]]]
.row[ 
.split-two[
...
<br> 
   ...
   - <span style="color:gray">Nodes in each connected component of $\mathcal{G}_\tau$ average their values and nodes outside of $\mathcal{G}_\tau$ do not update secret values; this produces new secret values $\mathbf{c}_k$.</span>
...
<br> 
   - Form $\mathcal{G}_\tau$, the edge-induced subgraph of $\mathcal{G}$ defined by edges in $\tau$.
   - Nodes in each connected component of $\mathcal{G}_\tau$ average their values and nodes outside of $\mathcal{G}_\tau$ do not update secret values; this produces new secret values $\mathbf{c}_k$.
...
.column.bg-main1[
### Future Work: A Bridge

<br> 

There is a natural bridge between many problems regarding consensus dynamics on networks and classical iterative methods from numerical linear algebra, which can be used to understand the convergence of the dynamics.
...
This technique may be exploited for .alert[other consensus dynamics problems on networks].


]

.column.bg-main4[.center[.stretch[
<br><br>
.row[ 
.split-two[
.column[.lil-stretch[<br><br>

...
.column[
  ...
  .row.bg-main2[.content.vmiddle[.font_medium[Greedy heuristics for .alert[large hypergraphs].] 
  ]] 
...
Can we do .alert[scalable] clustering with .alert2[statistical foundations]?
...
There's some nice theory one can do with the affinity function, but for today let's focus on the .alert[All-Or-Nothing] affinity. 
...
We usually assume $\omega_{k1} > \omega_{k0}$. 
]
...
Objective function can be written, after some algebra:

$$
Q \triangleq \sum_k \color{#ff5252}{\beta_k} \left[(\text{# homogeneous }k\text{-edges}) - \color{#21728f}{\gamma_k}\sum_j (\text{# sum of degrees in cluster } j)^k\right]
$$

$\color{#ff5252}{\beta_k} \triangleq \log \omega_{k1} - \log \omega_{k0}$. 
- .alert[**Size parameters**] control importance of edge sizes. 

$\color{#21728f}{\gamma_k} \triangleq \frac{1}{\beta_k}(\omega_{k1} - \omega_{k0})$.
- **.alert2[Resolution parameters]** control number of clusters
...
### Empirical data: .alert[school contact hypergraphs]. 
...
.column.bg-main1[
  ...
  We can derive scalable algorithms from generative models of clustered hypergraphs. 

  These algorithms inherit assumptions from their models. 

  These algorithms can outperform classic graph-based methods on data sets in which those assumptions are warranted. 
...
.column.bg-main1[
  ...
  What if our data is very Big®?

  ### This Time

  What if our data is very .alert[*sparse*]? 
  ...
  .alert2[Consequence]: degree of a node doesn't grow with the size of the hypergraph. 
]

.column[
  ### Fundamental Limits

  We say that a clustering algorithm .alert[**detects clusters**] in a cluster model if the algorithm **sometimes** returns labels **correlated** with the true ones. 



  There are sparse graph models for which detection is provably impossible, even though clusters are present!
...
### Greedy Methods ( $c_2 = c_3 = 5$ )

<img src="img/detectability-louvain.png" width=100%>

Greedy methods fail in large regions of parameter space! Can we do better?
...
BP has a fixed point that contains no info about the clusters. 

If this fixed point is .alert[stable], BP can converge to the uninformative solution. 
...
In a sparse HSBM, 
  ...
  - $\mathbf{C}_k$ is a matrix of parameters that depends on the statistical model $P$. 
  - $\mathbf{B}_k$ is a special matrix called the *Hashimoto operator*, restricted to edges of size $k$. 
...
.alert[**Conjecture**]: In a 2-group blockmodel with edge sizes $k_1,k_2,\ldots$, detection is possible if and only if $(p_{k_1},p_{k_2},\ldots)$ lies within the axis-aligned ellipsoid with centroid $(x_{k_1},x_{k_2},\ldots)$ and radii $(r_{k_1},r_{k_1}\ldots)$, where: 
...
### Just For Fun: Clustering Math Topics on StackExchange
...
Belief-propagation can give us analytical insights on the limits of clustering in hypergraphs. 

Proving these conjectures is likely to require fancy math. 

...
#### Here's what we learned. <br> <br>

.pink-bg[
A generative approach to hypergraph modularity clustering can lead to .embolden[interpretable objectives] and .embolden[fast algorithms].
]
.midi[<br>]
.yellow-bg[
...
- Seems to work well, interpretability / connection to generative models unclear. 
]
...
#### The **statistical view** of graph modularity. 
...
This approximation is exact when all clusters have the same sum of internal degrees. 
...
#### We propose a **hypergraph degree-corrected stochastic blockmodel**.
...
$$\Omega(a, a, a) \geq \Omega(a, a, b)\geq \Omega(a, b, c)\ldots$$ 
...
.midi[
Example: $\mathbf{z}_R = (a, a, a, b, c, c)$, $\mathbf{p} = (3, 2, 1)$. 
]
...
#### Optimization over ** $\mathbf{z}$ ** leads to a modularity-type objective. 
...
#### These parameters are **interpretable** and can be **estimated from data.** 
...
#### Nate wrote efficient **hypergraph maximum-likelihood Louvain** (HMLL).
.small[<br>]
.pink-bg[
1. All nodes start in their own cluster. 
2. Greedily agglomerates nodes to maximize modularity. 
3. Then, greedily agglomerates entire .embolden[clusters] of nodes.  
4. Estimate parameters $\Omega$ and $\{\theta_i\}$
5. Repeat!
]

Works for general $\Omega$, **highly scalable** for All-or-Nothing $\Omega$ due to algebraic simplifications.  
...
.pink-bg[
A generative approach to hypergraph modularity can lead to .embolden[fast, interpretable algorithms].
]
.midi[<br>]
.yellow-bg[
.embolden[Hypergraph] algorithms can succeed where .embolden[graph] algorithms necessarily fail. 
...
1. Alternative inference frameworks/algorithms for DCHSBM:
  - Bayesian Monte Carlo.
  - Belief propagation.
  - Alternative greedy heuristics.
2. Faster methods for general affinity functions $\Omega$. 

...
.yellow-highlight[
  ...
  .pink-highlight[
  .embolden[Random graphs in data science (today)]
  ]
]
...
Performance varies between model variants, but is generally competitive with best extant approaches. 
...
Suppose I measure degree-assortativity in some network data - the tendency of nodes with many neighbors to themselves be connected. I find a correlation coefficient of 0.3. Is that...high? Low? Interesting? Boring?
...
.footnote[.white-bg[Distinction between vertex- and stub-labeled models due to Fosdick et al. (2018), "Configuring random graph models with fixed degree sequences," *SIAM Review* ]]
...
.pink-bg[
1. .embolden[Sample]: Pick two edges $\Delta$ and $\Gamma$ u.a.r.
2. .embolden[Proposal]: Randomly resample nodes between $\Delta$ and $\Gamma$ while avoiding degeneracy
3. .embolden[Transition]: With probability $m_{\Delta}^{-1}m_{\Gamma}^{-1}$, accept swap, otherwise start over. 
]

**Theorem**: This Markov chain is ergodic and its invariant measure is the configuration model $\eta_{\mathbf{d}, \mathbf{k}}$.   
]
...
.white-bg[
.yellow-highlight[
The expected number of edges $w_{ij}$ between two nodes in a .embolden["random"] graph .embolden["with degree sequence d]" is 
...
.white-bg[
Estimating $\omega_{ij} = \mathbb{E}[W_{ij}]$ via Edge-Swap Monte Carlo is .embolden[expensive] (~3 days on personal equipment). 

How can we estimate efficiently expectations (e.g. for modularity) from $\eta$? 
]
---
...
.alert[
.embolden[Theorem]: $$\chi_{ij}\triangleq \mathbb{E}_\eta[\mathbb{I}(W_{ij} > 0)] = \frac{\beta_i\beta_j}{\sum_\ell \beta_\ell} + \text{small terms}$$
]
]
...
.white-bg[
.pink-bg[
.embolden[Theorem]: $$\omega_{ij}\triangleq \mathbb{E}_\eta[W_{ij}] = \frac{\chi_{ij}}{1 - \chi_{ij}} + \text{small terms}$$ 
]
...
.alert[
.embolden[Theorem]: Solution (if it exists) is unique on the set 
